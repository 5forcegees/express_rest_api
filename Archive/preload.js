...
// save this file as preload.js
//
// run like this: node ./preload.js {env}
//                              
...
(function (root, factory) {
	...
	var _p = api._p = api._p || {},
		...
		// curl -X POST -H "Content-Type: application/json" -H "Accept: application/json" -H "senderid: Rebellion" -H "channelid: CARE" -H "applicationid: ACUI" -H "Authorization: Bearer BRurmGyf9TTxVKXRZf9vIiqx6KOS" -H "Cache-Control: no-cache" -H "Postman-Token: 01b157c0-f82e-2fe8-5df2-5193f9029bb6" -d '{
		//   "referenceDataRequest": {
		//     "requestDate": "2015-05-15T00:00:00-09:00",
		//     "fromDateTime": "2015-05-15T00:00:00-09:00"
		//   }
		// }' "https://qat01.api.t-mobile.com/v2/productoffering/refdata/offerdetails"

		api.getPreload = function(env, token, cb) {
			if (env==="qat"){
				var method='POST';
				var host='qat01.api.t-mobile.com';
				var path='/v2/productoffering/refdata/offerdetails';
				...
				api.httpsCall(options, function(err,statusCode,headers,data, callLength	){
					if (err) { throw new Error(err+' error getting preload '+statusCode+JSON.strinigy(data));}
		...
		api.cli = function(args) {
			...
			if (env === 'qat') {
				...
				api.getCoreToken(env,function(err, tokenResponse){
					...
					api.getPreload(env, coreToken, function(err, preload){
						var output = {
							name: "core pre-load call",
							httpstatus: preload.statusCode,
							status: (preload.statusCode === 200) ? (preload.body+0>100000) ? "up" : "questionable" : "down",
							api_checked_date: new Date(),
							elapsed_time: preload.callLength,
							...
							response: "large response, size = "+preload.body
						};
